...
class BuildStruct:
    ...
    def loadGPSDataset(self, csvGyroPath):
        gpsDF = pd.read_csv(csvGyroPath, names=["giro","long","lat","time"])
...
b = BuildStruct()
df = b.loadImagesDataset("F:\\19_08_22")

df.to_csv('E:\\street_view_plugin\\imagens_atual_19.csv', index=False)

""" df = pd.read_csv('E:\\street_view_plugin\\fotos_final_18.csv', header=0)
result = b.removeNearPoints(df)
result.to_csv('E:\\street_view_plugin\\fotos_final_18_fix1.csv', index=False) """

# var = 0.24727201016696612

# for i, row in df.iterrows():
#     if not pd.isnull(row['timestamp']):
#         continue
#     df.at[i, 'timestamp'] = 1660836389 + (i * var)
# df.to_csv('./18_gps_info_FIX.csv', index=False)
#45615